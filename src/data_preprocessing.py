...
class DataProcessor:
      ...
      def preprocess_data(self, df):
            try:
                  ...
                  skewed_threshold = self.config["data_processing"]["skewness_threshold"]
                  skewness = df[num_cols].apply(lambda x: x.skew())

                  for col in skewness[skewness > skewed_threshold].index:
                        df[col] = np.log1p(df[col])
                  
                  return df
            except Exception as e:
                  logger.error("Error during preprocess step")
      ...
      def select_features(self, df):
            try:
                  ...
                  number_features_to_select = self.config["data_processing"]["no_of_features"]

                  top_ten_features = top_feature_importance_df['feature'].head(number_features_to_select).tolist()

                  logger.info(f"{top_ten_features} are selected as top features")

                  top_ten_df = df[top_ten_features + ['booking_status']]
      ...
      def process(self):
            try:
                  ...
                  # Select features
                  train_df = self.select_features(train_df) #no need to select features for test data because they are same for both train and test
